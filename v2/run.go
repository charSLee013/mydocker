...
func Run(tty bool, comArray []string, res *subsystems.ResourceConfig, programURL string, envSlice []string) {

	// 生成containerID
	containerID := randStringBytes(16)

	// 创建隔离环境,并通过 pipe 的方式传递参数
	parent,writePipe := driver.NewParentProcess(tty, containerID,programURL,envSlice)
	if parent == nil {
	...
	if tty{
		// 等待前台进程完成
		parent.Wait()
		driver.DeleteWorkSpace(containerID)
	}
}
