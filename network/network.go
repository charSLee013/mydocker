...
var (
	defaultNetworkPath = "/var/lib/gdocker/network/network"
...
func Init(sugar *zap.SugaredLogger) error {
	...
	Sugar.Infof("networks : %v", networks)
	return nil
}
...
func (nw *Network) load(dumpPath string) error {
	...
	if err != nil {
		Sugar.Errorf("Error load nw info", err)
		return err
	}
	return nil
}
...
func configPortMapping(ep *Endpoint, cinfo *container.ContainerInfo) error {
	for _, pm := range ep.PortMapping {
		...
		if err != nil {
			Sugar.Errorf("iptables Output, %v", output)
			continue
		}
	}
	return nil
